          ...
          \begin{enumerate}
              ...
              \item a $j \to i$ mutation (with $j$ possibly equal to $i$), with
                    ...
                    \[
                        \kappa_{ij} =
                        \begin{cases}
                            \frac{n_{j,t} + \mu \psi_{j}}{\mnorm{\vec{n}_{t}} - 1 + \mu} & j \neq i \text{,} \\
                            \frac{n_{j,t} - 1 + \mu \psi_{j}}{\mnorm{\vec{n}_{t}} - 1 + \mu} & j = i \text{,}
                        \end{cases}
                    \]
                    and $\vec{\psi}$ is the stationary distribution associated
                    with $\mat{P}$;
          \end{enumerate}
    \item Update the counts for each type,
          \[
              \vec{n}_{t+1} =
              \begin{cases}
...
\[
    \log b = \log\!\left[ \frac{\mnorm{\vec{n}_{\rho}}!\,\Gamma\!\left( \mu \right)}{\Gamma\!\left( \mu + \mnorm{\vec{n}_{\rho}} \right)} \right] + \sum_{i=1}^{d} \log\!\left[ \frac{\Gamma\!\left( n_{i,\rho} + \mu\,\psi_{i} \right)}{n_{i,\rho}!\,\Gamma\!\left( \mu\,\psi_{i} \right)} \right]\!\text{,}
\]
